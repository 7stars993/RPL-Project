...
function editOrders($kode_transaksi, $data, $files) {
  $kode_makanan = $data['kode_makanan'];
  $sisa_stok = $data['sisa_stok'];
  $metode_bayar = $data['metode_bayar'];
  $bukti_bayar = htmlspecialchars($files['gambar']['name']);
  $total = $data['total'];

  try{
    	// Update tabel transaksi
		$statement = DB->prepare("UPDATE transaksi SET STATUS = :status, WAKTU_BAYAR = :waktu_bayar, METODE_BAYAR = :metode_bayar, BUKTI_BAYAR = :bukti_bayar, TOTAL = :total WHERE KODE_TRANSAKSI = :kode_transaksi");
		$statement->bindValue(':status', 1);
		$statement->bindValue(':bukti_bayar', $bukti_bayar);
		$statement->bindValue(':metode_bayar', $metode_bayar);
